...
// Model for NewsItem entity
    models.NewsItem = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/ne/webresources/nieuws.model.newsitem/",
        defaults: {
            author: "",
            id: "",
            title: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('author');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/ne/webresources/nieuws.model.newsitem/';
            }
    ...
    // Collection class for NewsItem entities
    models.NewsItemCollection = Backbone.Collection.extend({
        model: models.NewsItem,
        url: "http://localhost:8080/ne/webresources/nieuws.model.newsitem/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.NewsItemCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-newsitem-list-itemis template identifier for item
                        templateName: '#tpl-newsitem-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.newsitem = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.newsitem,
                    // tpl-newsitem-details is template identifier for chosen model element
                    templateName: '#tpl-newsitem-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.NewsItem();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-newsitem-details is a template identifier for chosen model element
                templateName: '#tpl-newsitem-details',
