...
public:
	...
	Graaf keerOm();
	void samenhangZoeken();
	std::vector<int> componentNummers;
...
protected:
	...
	void recursiePostOrder(vector<int> &postOrders, int huidig, int &postOrder);
	void recursieSamenHang(vector<bool> &gezien, int huidig, int componentNummer);
protected:
	//datavelden
...
template<RichtType RT>
void Graaf<RT>::recursiePostOrder(vector<int> &postOrders, int huidig, int &postOrder) {
	for (std::map<int, int>::const_iterator it = knopen[huidig].begin(); it != knopen[huidig].end(); it++) {
		if (postOrders[it->first] != -1) {
			recursiePostOrder(postOrders, it->first, postOrder);
		}
	}
	postOrders[postOrder++] = huidig;
}

template<RichtType RT>
vector<int> Graaf<RT>::postOrder() {
	vector<int> postOrders;
	for (int i = 0; i<knopen.size(); i++){
		postOrders.push_back(-1);
	}
	int postOrder = 0;
	for (int i = 0; i<knopen.size(); i++){
		if (postOrders[i] == -1) {
			recursiePostOrder(postOrders, i, postOrder);
		}
	}
	return postOrders;
}
...
template<RichtType RT>
void Graaf<RT>::recursieSamenHang(vector<bool> &gezien, int huidig, int componentNummer) {
	cout << componentNummer<<endl;
	for (std::map<int, int>::const_iterator it = knopen[huidig].begin(); it != knopen[huidig].end(); it++) {
		if (!gezien[it->first]) {
			gezien[it->first] = true;
			componentNummers[it->first] = componentNummer;
			recursieSamenHang(gezien, it->first, componentNummer);
		}
	}
}

template<RichtType RT>
void Graaf<RT>::samenhangZoeken() {
	Graaf<RT> omgekeerde = keerOm();
	vector<int> postOrders = omgekeerde.postOrder();
	vector<bool> gezien;
	for (int i = 0; i<knopen.size(); i++){
		gezien.push_back(false);
	}
	int componentNummer = 0;
	for (int i = postOrders.size() - 1; i >= 0; i--) {
		if (!gezien[postOrders[i]]) {
			recursieSamenHang(gezien, postOrders[i], componentNummer++);
		}
	}
}
...
Graaf<RT> Graaf<RT>::keerOm(){
	Graaf<RT> omgekeerde(0);
	for (int i = 0; i<knopen.size(); i++){
		omgekeerde.voegKnoopToe();
	}
	for (int i = 0; i<knopen.size(); i++){
		for (std::map<int, int>::const_iterator it = knopen[i].begin(); it != knopen[i].end(); it++) {
			omgekeerde.voegVerbindingToe(it->first, i);
		}
	}
	return omgekeerde;
}
