...
sockaddr_in maak_adres_ipv4(const std::string &servernaam, short poort, int socktype)
{
    ...
    //TBD: kopieer nuttig resultaat naar aparte struct en roep dan freeaddrinfo op
    sockaddr_in result = *(sockaddr_in *)res->ai_addr;
...
{
    if(sd<0)
        throw NetwerkExceptie("Problemen met sluiten(): "+std::string(strerror(errno)));
    //Close(sd);
}
...
sockaddr_in6 maak_server_adres_ipv6(short poort)
{
    ...
    servaddr.sin6_port = htons(poort);      //htons!   
    return servaddr;
}
